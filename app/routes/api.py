...
def _apply_ytdlp_cookies(ydl_opts):
    ...
    if stripped.startswith("["):
        cookies_text = _json_cookies_to_netscape(cookies_text)
    cookie_path = Path(tempfile.gettempdir()) / "wedflix_ytdlp_cookies.txt"
    cookie_path.write_text(cookies_text, encoding="utf-8")
    ydl_opts["cookiefile"] = str(cookie_path)


def _json_cookies_to_netscape(cookies_text):
    rows = json.loads(cookies_text)
    lines = [
        "# Netscape HTTP Cookie File",
        "# Generated by Wedflix from browser-exported JSON cookies.",
    ]
    for cookie in rows:
...
def session_signup_request_otp():
    ...
    try:
        ...
        current_app.logger.exception("Could not send signup OTP email to %s: %s", email, exc)
        smtp_error = exc
        sent = False

    is_debug = _is_debug_env()
    if not sent and not is_debug:
        _log_otp_delivery_failure(email, "signup", smtp_error)
...
def session_password_request_otp():
    ...
    try:
        ...
        current_app.logger.exception("Could not send password reset OTP email to %s: %s", email, exc)
        smtp_error = exc
        sent = False

    is_debug = _is_debug_env()
    if not sent and not is_debug:
        _log_otp_delivery_failure(email, "password_reset", smtp_error)
        return jsonify({"error": "Could not send OTP right now. Please try again later."}), 503
